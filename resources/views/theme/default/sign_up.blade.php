				...
				<div class="auth-form">
					<form action="{{ route('register') }}" method="post" autocomplete="off">
						...
						<div class="row">
							...
							<div class="col-md-12">
								<div class="single-form form-group">
									<input type="text" name="mobile" value="{{ old('mobile') }}" placeholder="{{ _lang('Your Mobile Number') }}" required>
							...
							<div class="col-md-6">
								<div class="single-form form-group {{ $errors->has('district_id') ? 'has-error' : '' }}">
									<select class="select2 form-control" name="district_id" id="district_id" required>
										<option value="">{{ _lang('Please Select District') }}</option>
									...
									@if ($errors->has('district_id'))
	                                    <div class="help-block with-errors">{{ $errors->first('district_id') }}</div>
	                                @endif
								</div>
							</div>

							<div class="col-md-6">
                                <div class="single-form form-group {{ $errors->has('upazila_id') ? 'has-error' : '' }}">
                                    <select class="form-control" name="upazila_id" id="upazila_id" required>
                                        <option value="">{{ _lang('Please Select Upazila') }}</option>
                                    </select>
                                    @if ($errors->has('upazila_id'))
                                        <div class="help-block with-errors">{{ $errors->first('upazila_id') }}</div>
                                    @endif
                                </div>
                            </div>
                            ...
                            <div class="form-group{{ $errors->has('CaptchaCode') ? ' has-error' : '' }}">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="text" id="CaptchaCode" name="CaptchaCode" required="required">
							...
							<div class="col-md-12">
								<div class="single-form form-group text-center">
									...
									{{-- <br><a href="{{ url('register/client_signup') }}" class="btn-link mt-2">{{ _lang('Guardian Sign Up') }}</a> --}}
								</div> <!-- single form -->
							</div>
...
<script type="text/javascript">
	$("#district_id").change(function(){
		$.ajax({
			url: "{{ route('signup.upazila') }}?district_id=" + $(this).val(),
			method: 'GET',
