	...
	<div class="card-body">
	  <h4 class="d-none panel-title">{{ _lang('View User') }}</h4>
	   ...
	   <table class="table table-bordered">
			...
			<tr><td>{{ _lang('Status') }}</td><td>{!! $user->company->status == 1 ? (status(_lang('Active'), 'success')) : (status(_lang('In-Active'), 'danger')) !!}</td></tr>
			@if($user->user_type == 'user')
		        ...
		        <tr>
		        	<td>{{ _lang('Membersip Type') }}</td><td>{!! $user->company->membership_type == 'trial' ? (status(ucwords($user->company->membership_type), 'danger')) : (status(ucwords($user->company->membership_type), 'success')) !!}</td>
		        </tr>
			@endif
	    </table>
	    ...
	    @if($user->user_type == 'user')
		    <table class="table table-striped">
		    	...
		    	<tr>
		    		...
		    		<td class="text-center">{{ $user->company->drivers_limit }}</td>
		    	</tr>
		    	<tr>
		    		<td>{{ _lang('Vehicle Limit') }}</td>
		    		<td class="text-center">{{ $user->company->vehicles_limit }}</td>
		    	</tr>
		    	<tr>
