...
<html lang="en">
    ...
    <head>
	    ...
	    <link href="{{ asset('public/backend/plugins/toastr/toastr.css') }}" rel="stylesheet" type="text/css" />
	    <link href="{{ asset('public/backend/plugins/extensions/toastr.css') }}" rel="stylesheet" type="text/css" />
		...
		<link rel="stylesheet" href="{{ asset('public/backend/assets/css/responsive.css?v=1.3') }}">
		<link rel="stylesheet" href="{{ asset('public/backend/assets/css/pickadate/pickadate.css') }}">

		<!-- Modernizr -->
		<script src="{{ asset('public/backend/assets/js/vendor/modernizr-2.8.3.min.js') }}"></script>

		<!--Chat Widget-->
		<link href="{{ asset('public/backend/assets/css/chat-widget.css?v=1.3') }}" rel="stylesheet" type="text/css" />
		...
		@if(get_company_option('backend_direction',get_option('backend_direction')) == "rtl")
			...
			<link rel="stylesheet" href="{{ asset('public/backend/assets/css/rtl/style.css?v=1.3') }}">
		@endif
    ...
    <body>
		...
		<div class="page-container">
			...
			<div class="sidebar-menu" style="background-color: rgb(3, 3, 3);">

				<div class="sidebar-header bg-primary">
					<div class="logo">
                        <a href="#"><img src="{{ get_company_logo() }}" class="company-logo" alt="logo"></a>
                        <span class="text-white ml-1 d-inline-block">JobPulse</span>
						{{-- @if (Auth::user()->company->business_name)
						<span class="text-white ml-1 d-inline-block">Bangladesh handloom education and training Institute Alumni Association</span>
						@else
					...
					<div class="menu-inner">
						<nav>
							<ul class="metismenu" id="menu">
							    <li><a href="{{ url('admin/dashboard') }}"><i class="ti-dashboard"></i> <span>{{ _lang('Dashboard') }}</span></a></li>
	                            ...
	                            <li>
									<div class="user-profile">
										...
										<div class="dropdown-menu">
                                           ...
                                           @endif
                                             @if(Auth::user()->user_type == 'admin' && get_option('membership_system') == 'enabled')
                                             <a class="dropdown-item" href="{{ url('profile/edit') }}"><i class="ti-settings text-muted mr-2"></i> {{ _lang('Profile Settings') }}</a>
											@endif
					...
					@if(has_membership_system() == 'enabled' && $user->user_type == 'user')
						...
						@if( $user->company->membership_type == 'trial' && membership_validity() > date('Y-m-d'))
							<div class="alert alert-warning">
							   <b>{{ _lang('You Are Currenly Using Trial Account and it is valid till '.membership_validity()) }}&emsp;<a href="{{ url('membership/extend') }}" class="btn btn-danger btn-xs">{{ _lang('Upgrade Now') }}</a></b>
							</div>
						@endif
		...
		<script src="{{ asset('public/backend/assets/js/vendor/pickadate/picker.js') }}"></script>
		<script src="{{ asset('public/backend/assets/js/vendor/pickadate/picker.date.js') }}"></script>
		<script src="{{ asset('public/backend/assets/js/vendor/pickadate/picker.time.js') }}"></script>
