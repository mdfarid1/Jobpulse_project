...
if ( ! function_exists('teacher_name')){
	...
	{
		$name=\App\User::select('name')->where('id',$id)->where('user_type','staff')->get();
		return $name[0]->name;
	}
}
...
if ( ! function_exists('subject_name')){
	...
	{
		$name=\App\Models\Subject::select('name')->where('id',$id)->get();
		return $name[0]->name;
	}
}
...
if ( ! function_exists('booking_status')){
	function booking_status($status,$id)
	{
		// dd($status);
			$uc_status = ucwords($status);
		if($status == 'booked'){
			return "<div class='btn-group'><div class='badge badge-secondary dropdown'><a class='dropdown-toggle' data-toggle='dropdown' href='#''><span class='badge badge-secondary'>$uc_status&nbsp;&nbsp;<i class='fas fa-angle-down'></i></span></a>
			 <div class='dropdown-menu' x-placement='top-start' style=''>
			   <a class='dropdown-item csts' data-id='".$id."' data-sts='ongoing' href='#' style=''>On-going</a>
         <a class='dropdown-item csts' data-id='".$id."' data-sts='delivered' href='#' style=''>Delivered</a>
			 </div>
			...
			return "<div class='btn-group'><div class='badge badge-warning dropdown'><a class='dropdown-toggle' data-toggle='dropdown' href='#''><span class='badge badge-warning'>$uc_status&nbsp;&nbsp;<i class='fas fa-angle-down'></i></span></a>
			 <div class='dropdown-menu' x-placement='top-start'>
			  <a class='dropdown-item csts' data-id='".$id."' data-sts='booked' href='#' style=''>Booked</a>
        <a class='dropdown-item csts' data-id='".$id."' data-sts='delivered' href='#' style=''>Delivered</a>
			 </div>
...
if ( ! function_exists('sms_send_customer')){
	function sms_send_customer($client_id, $invoice_number)
	{
		//$client=$invoice['client_id'];
		$company_id = company_id();
		$client=\App\Contact::where('contacts.id','=',$client_id)->pluck('contact_phone');
		//$company_number=Auth::user()->mobile;
		$api_username = \App\CompanySetting::where('company_id',$company_id)
						->where('name','sms_api_username')
						->first(['value'])->value;
		$api_password = \App\CompanySetting::where('company_id',$company_id)
						->where('name','sms_api_password')
						->first(['value'])->value;
		$text=\App\SMS::select('s_m_s.body')->where('name','booking_sms_customer')->first(['body'])->body;
    $text=strip_tags($text);

    $text = str_replace('[link]',url('c/i/'.$invoice_number), $text);
        // dd($text);
    $url = "http://66.45.237.70/api.php";
		$number="$client[0]";//"88017,88018,88019";
		$data= array(
...
if ( ! function_exists('sms_send_driver')){
	function sms_send_driver($driver_id, $invoice_date)
	{
		//$client=$invoice['client_id'];
		$company_id = company_id();
		$driver=\App\Driver::where('drivers.id','=',$driver_id)->pluck('mobile');
		//$company_number=Auth::user()->mobile;
		$api_username = \App\CompanySetting::where('company_id',$company_id)
						->where('name','sms_api_username')
						->first(['value'])->value;
		$api_password = \App\CompanySetting::where('company_id',$company_id)
						->where('name','sms_api_password')
						->first(['value'])->value;
		$text=\App\SMS::select('s_m_s.body')->where('name','booking_sms_driver')->first(['body'])->body;
    $text=strip_tags($text);

    $text = str_replace('[date]',$invoice_date, $text);
        // dd($text);
    $url = "http://66.45.237.70/api.php";
		$number="$driver[0]";//"88017,88018,88019";
		$data= array(
		'username'=>"$api_username",
		'password'=>"$api_password",
		'number'=>"$number",
...
if ( ! function_exists('has_feature'))
{
    function has_feature( $feature, $limit = false ){
		...
		if( $company->$feature == 'Yes' && $company->feature == 'Unlimited'){
           return true;
		}else if( $company->$feature == 'No' || $company->$feature == 'NULL'){
		   return false;
		}else{
			return true;
		}
...
if ( ! function_exists('sms_send_guardian')){
	function sms_send_guardian($user, $body)
	{
		$enter=\Carbon\Carbon::now();
		$entry_time=date("d-m-Y g:i A", strtotime($enter));
		$company_id = company_id();
		// $client=\App\Contact::where('contacts.id','=',$student_id)->pluck('contact_name');
		//$company_number=Auth::user()->mobile;
		$text=strip_tags($body);
		$api_url = \App\CompanySetting::where('company_id',$company_id)
						->where('name','sms_api_link')
						->first(['value'])->value;
	// 	$text=\App\SMS::select('s_m_s.body')->where('name','booking_sms_customer')->first(['body'])->body;
    // $text=strip_tags($text);

    // $text = "$student_name is enter at $entry_time in class/school.";
        // dd($user);
        $url = $api_url;
	    $number="$user->contact_phone";//"88017,88018,88019";
		$data= array(
            'number'=>"$number",
            'message'=>"$text"
        );
...
if ( ! function_exists('sms_send_guardian_exit')){
	function sms_send_guardian_exit($user, $body)
	{
		//$client=$invoice['client_id'];
		$exit=\Carbon\Carbon::now();
...
if ( ! function_exists('sms_send_subject_student')){
	function sms_send_subject_student($students,$subject_id)
	{

		$c=count($students);

		for ($i=0; $i < $c; $i++) {
			// array_push($a,$students[$i]->contact_phone);
		$company_id = company_id();

        $replace = array(
                    '{student_name}'=>$students[$i]->contact_name,
                    '{id_no}'=>$students[$i]->reg_no,
                    '{company_short_name}' =>get_company_option('institute_short_name'),
                );
        $template = \App\CompanySmsTemplate::where('name','Absent')->first();
        ...
        $student_name=$students[$i]->contact_name;
        $company_short_name = get_company_option('institute_short_name');
		$text = "{$student_name} আজকে {$subject} ক্লাসে অনুস্পস্থিত ছিল।{$company_short_name}";
			// dd($text);
		$url = $api_url;
		$number=$students[$i]->contact_phone;//"88017,88018,88019";
		$data= array(
		'number'=>"$number",
		'message'=>"$text"
		);
...
if ( ! function_exists('sms_send_student')){
	function sms_send_student($students)
	{
...
if ( ! function_exists('sms_subscription')){
	function sms_subscription($user,$body)
	{
		//$client=$invoice['client_id'];
       $company_id = company_id();
	   $text=strip_tags($body);
		...
		$number="$user->mobile";//"88017,88018,88019";
        $url ="http://bulksmsbd.net/api/smsapi?api_key=IBPJuXuI2JXlzglJkS3Z&type=text&senderid=8809612440788";
		$data= array(
		'number'=>"$number",
		'message'=>"$text"
		);
...
if(!function_exists('sms_subscription_msg_own')){
    function sms_subscription_msg_own($user,$payment){

	   $text="সাবস্ক্রিপশন চার্জ বাবদ {$user->company->business_name} থেকে {$payment->amount} টাকা গ্রহণ করা হয়েছে।  ";

        // dd($text);
		$number="01689655055";//"88017,88018,88019";
        $url ="http://bulksmsbd.net/api/smsapi?api_key=IBPJuXuI2JXlzglJkS3Z&type=text&senderid=8809612440788";
		$data= array(
		'number'=>"$number",
		'message'=>"$text"
...
if ( ! function_exists('sms_send_self')){
	function sms_send_self($user)
	{
		...
		$text = "{$user->name} has registered as new customer institute name-{$user->company->business_name}. Thanks-Attendance Keeper PVT. Ltd";
		$url = "http://bulksmsbd.net/api/smsapi?api_key=IBPJuXuI2JXlzglJkS3Z&type=text&senderid=8809612440788";
		$number="01689655055";//"88017,88018,88019,01689655055";
		// dd($number);
		$data= array(
		'number'=>"$number",
		'message'=>"$text"
