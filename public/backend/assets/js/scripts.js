(function($) {
	...
	*/
    ...
    $(document).on('click','.btn-atndc-2',function(){
		//Sweet Alert for delete action
		Swal.fire({
		  title: $lang_alert_title,
		  text: $lang_atndc_message,
		  icon: 'warning',
		  showCancelButton: true,
		  confirmButtonColor: '#3085d6',
		  cancelButtonColor: '#d33',
		  confirmButtonText: $lang_confirm_atndc_button_text,
		  cancelButtonText: $lang_cancel_button_text
		}).then((result) => {
		  if (result.value) {
			$(this).closest('form').submit();
	...
	$(document).on('change', '#sms_template_related_to', function(){
			if($(this).val() == 'enter'){
				$("#enter-paremeter").removeClass('d-none');
				$("#exit-paremeter").addClass('d-none');
			}else if($(this).val() == 'exit'){
				$("#exit-paremeter").removeClass('d-none');
				$("#enter-paremeter").addClass('d-none');
			}
	});
